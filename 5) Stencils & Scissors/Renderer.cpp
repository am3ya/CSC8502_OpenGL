...
void Renderer::RenderScene() {
	...
	if (usingStencil) {
		...
		glStencilFunc(GL_ALWAYS, 2, ~0); //Check this line if the program doesn't work, not sure if that's supposed to be a ~ before the 0
		...
		glStencilFunc(GL_EQUAL, 2, ~0); //Again, not sure if that's supposed to be a ~
